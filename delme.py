...
def dashi_compress_reconstruct(
    omega,
    tau=0.35,
    smooth_k=11,
    q=0.05,
    ...
    """
    DASHI structural codec for a scalar field (vorticity ω)

    Returns:
        omega_rec : reconstructed field
        stats     : dict with storage + accuracy metrics
    """

    H, W = omega.shape
    raw_bits = H * W * 32  # float32 baseline

    # ---------- structural layer ----------
    omega_base = smooth2d(omega, k=smooth_k)
    X = omega - omega_base
    ...
    m = (s_star != 0)
    sigma = s_star[m]              # ±1 values
    idx = np.argwhere(m)           # coordinates

    n_support = idx.shape[0]

    # ---------- skeleton amplitudes ----------
    # sample representative ω values on support
    amps = omega[m]
    amps_q = np.round(amps / q).astype(np.int16)

    # ---------- reconstruct structure ----------
    omega_hat = np.zeros_like(omega)
    omega_hat[m] = amps_q.astype(float) * q

    # smooth to get structural field
    omega_hat = smooth2d(omega_hat, k=smooth_k)

    # ---------- residual ----------
    resid = omega - omega_hat
    band = m  # simple choice; could dilate

    resid_q = np.zeros_like(resid)
    resid_q[band] = np.round(resid[band] / q)

    omega_rec = omega_hat + resid_q * q

    # ---------- storage accounting ----------
    bits_idx = n_support * (np.ceil(np.log2(H)) + np.ceil(np.log2(W)))
    bits_sign = n_support * 1
    bits_amp = n_support * 16          # int16
    bits_resid = np.sum(band) * 16     # int16 residuals
    bits_params = 64                   # tau, q, smooth_k etc.
    ...
    return omega_rec, stats
qs = [0.02, 0.05, 0.1, 0.2]

for q in qs:
    omega_rec, stats = dashi_compress_reconstruct(
        omega_gated,
        tau=0.35,
        smooth_k=11,
        q=q,
