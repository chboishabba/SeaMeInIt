...
class LegalityConfig:
    ...
    def __init__(
        ...
        object.__setattr__(
            self,
            "radii",
            dict(radii) if radii is not None else dict(DEFAULT_RADII),
        )
        default_pairs = (
            ("spine1", "spine2"),
            ("spine2", "spine3"),
        ...
        object.__setattr__(
            self,
            "ignore_pairs",
            tuple(ignore_pairs) if ignore_pairs is not None else default_pairs,
        )
    ...
    for index, a in enumerate(names):
        ...
        for b in names[index + 1 :]:
            ...
            if np.linalg.norm(pa) < 1e-6 and np.linalg.norm(pb) < 1e-6:
                continue
            penetration = max(0.0, cfg.alpha * (ra + rb) - float(np.linalg.norm(pa - pb)))
            if penetration > 0:
                violations += 1
